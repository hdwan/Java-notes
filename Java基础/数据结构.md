...
`Collection` 接口有 `3` 种子类型，`List`、`Set` 和 `Queue`。
...
**LinkedList实质是一个链表。**底层通过双向链表实现，双向链表的每个节点用内部类`Node`表示。`LinkedList`通过`first`和`last`引用分别指向链表的第一个和最后一个元素。当链表为空的时候二者指向`null`。
